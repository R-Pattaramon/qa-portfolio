...
TC031 - Cart Page - ตรวจสอบกด Check out โดยไม่มีสินค้า
    [Documentation]    ตรวจสอบการกดปุ่ม Shopping cart และ Check out โดยไม่ได้เพิ่มสินค้าเข้าตะกร้า
    [Tags]    TC031    Cart    Negative
    Login As Standard User
    Go To Shopping Cart
    Click Checkout Button
    Page Should Not Contain    Checkout: Your Information

TC032 - Cart Page - กด Link ชื่อสินค้าในหน้า Your Cart
...
TC033 - Cart Page - กดปุ่ม Remove ในหน้า Your Cart
...
TC037 - Checkout Info - กรอก First Name ที่ไม่ถูกต้อง
    [Documentation]    ตรวจสอบการกรอก Field First Name ในรูปแบบที่ไม่ถูกต้อง (@ชื่อ)
    [Tags]    TC037    CheckoutInfo    FirstName    Negative
    Login As Standard User
    Add Backpack To Cart And Go To Cart
    Click Checkout Button
...
TC038 - Checkout Info - กรอก Last Name ที่ถูกต้อง
    [Documentation]    ตรวจสอบการกรอก Field Last Name ในรูปแบบที่ถูกต้อง
    [Tags]    TC038    CheckoutInfo    LastName    Positive
    Login As Standard User
    Add Backpack To Cart And Go To Cart
    Click Checkout Button
...
TC039 - Checkout Info - กรอก Last Name ที่ไม่ถูกต้อง
    [Documentation]    ตรวจสอบการกรอก Field Last Name ในรูปแบบที่ไม่ถูกต้อง (@นามสกุล)
    [Tags]    TC039    CheckoutInfo    LastName    Negative
    Login As Standard User
    Add Backpack To Cart And Go To Cart
    Click Checkout Button
...
TC040 - Checkout Info - กรอก Zip/Postal Code ที่ถูกต้อง (ตัวเลข)
    [Documentation]    ตรวจสอบการกรอก Field Zip/Postal Code ในรูปแบบที่ถูกต้อง (ตัวเลข)
    [Tags]    TC040    CheckoutInfo    PostalCode    Positive
    Login As Standard User
    Add Backpack To Cart And Go To Cart
    Click Checkout Button
...
TC041 - Checkout Info - กรอก Zip/Postal Code ที่ไม่ถูกต้อง
    [Documentation]    ตรวจสอบการกรอก Field Zip/Postal Code ในรูปแบบที่ไม่ถูกต้อง (ab@1235)
    [Tags]    TC041    CheckoutInfo    PostalCode    Negative
    Login As Standard User
    Add Backpack To Cart And Go To Cart
    Click Checkout Button
...
TC043 - Checkout Info - ไม่กรอก First Name แล้วกด Continue
...
TC044 - Checkout Info - ไม่กรอก Last Name แล้วกด Continue
...
TC045 - Checkout Info - ไม่กรอก Postal Code แล้วกด Continue
...
TC046 - Checkout Info - ไม่กรอกข้อมูลใดเลยแล้วกด Continue
    [Documentation]    ตรวจสอบการกด Continue เมื่อไม่กรอกข้อมูลใดเลย แสดง First Name required
    [Tags]    TC046    CheckoutInfo    RequiredField    Negative
    Login As Standard User
    Add Backpack To Cart And Go To Cart
    Click Checkout Button
    Click Continue Button
...
TC047 - Checkout Info - กรอกข้อมูลถูกต้องและกด Continue เข้าสู่หน้า Overview
    [Documentation]    ตรวจสอบการกรอกข้อมูลครบและกด Continue เข้าสู่หน้า Checkout: Overview
    [Tags]    TC047    CheckoutInfo    CheckoutOverview    Positive
    Login As Standard User
    Add Backpack To Cart And Go To Cart
    Click Checkout Button
...
TC048 - Checkout Overview - กด Link ชื่อสินค้าในหน้า Overview
...
TC049 - Checkout Overview - กดปุ่ม Cancel กลับสู่หน้า Products
    [Documentation]    ตรวจสอบการกดปุ่ม Cancel ในหน้า Checkout: Overview กลับสู่หน้า Products
    [Tags]    TC049    CheckoutOverview    Positive
    Login As Standard User
    Complete Checkout Flow
    Click Cancel On Overview Page
    Element Text Should Be    css=.title    Products

TC050 - Checkout Overview - กดปุ่ม Finish เข้าสู่หน้า Checkout Complete
    [Documentation]    ตรวจสอบการกดปุ่ม Finish เข้าสู่หน้า Checkout: Complete! สั่งซื้อสำเร็จ
    [Tags]    TC050    CheckoutOverview    CheckoutComplete    Positive
    Login As Standard User
    Complete Checkout Flow
    Click Finish Button
    Verify Checkout Complete Page

TC051 - Checkout Complete - กดปุ่ม Back Home กลับสู่หน้า Products
    [Documentation]    ตรวจสอบการกดปุ่ม Back Home ในหน้า Checkout: Complete! กลับสู่หน้า Products
    [Tags]    TC051    CheckoutComplete    Positive
    Login As Standard User
    Complete Checkout Flow
    Click Finish Button
